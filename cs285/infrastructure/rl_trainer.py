...
class RL_Trainer(object):
    ...
    def run_training_loop(self, n_iter, collect_policy, eval_policy,
        ...
        self.iter_start_time = self.start_time
        ...
        for itr in range(n_iter):
            ...
            if itr % print_period == 0:
                curr_time = time.time()
                time_this_iter = curr_time - self.iter_start_time
                time_so_far = curr_time - self.start_time
                ave_time_per_iter = time_so_far / (itr // print_period+1)
                est_time_to_go = (n_iter - itr - 1) // print_period * ave_time_per_iter

                print('Time this iter: ', time.strftime('%H:%M:%S', time.gmtime(time_this_iter)))
                print('Time so far: ', time.strftime('%H:%M:%S', time.gmtime(time_so_far)))
                print('Ave time per iter: ', time.strftime('%H:%M:%S', time.gmtime(ave_time_per_iter)))
                print('Est time to go: ', time.strftime('%H:%M:%S', time.gmtime(est_time_to_go)))
                self.iter_start_time = curr_time
